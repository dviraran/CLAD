#!/usr/bin/env python3
"""
Test an LLM on medical consultation scenarios with CONCISE response requirements.

This is a modified version of test_llm.py that instructs the LLM to keep
final recommendations under 1500 characters. Used for the verbosity experiment.

Usage:
    # Test with GPT-5.2 concise mode
    python scripts/test_llm_concise.py --provider openai --model gpt-5.2 --case-id bailii-qb-2021-169-html

    # Test specific cases from a file
    for case_id in $(cat data/experiment_concise_cases.txt); do
        python scripts/test_llm_concise.py --provider openai --model gpt-5.2 --case-id "$case_id"
    done
"""
...
# LLM system prompt for medical consultation - WITH CONCISENESS REQUIREMENT
...
Your FINAL assessment and recommendations MUST be 1500 characters or less.
Be concise and prioritize the most important clinical actions.
Do not include lengthy explanations - focus on actionable recommendations.
...
def get_llm_response(client, provider: str, model: str, messages: list[dict]) -> str:
    ...
    if provider in ["openai", "together", "groq", "huggingface", "openrouter", "dr7", "vertexai"]:
...
def run_consultation(
    ...
    # Start session - use "-concise" suffix to distinguish from original
    llm_name = f"{provider}/{model}-concise"
    start_data = {"llm_name": llm_name}
    if case_id:
    ...
    print(f"LLM: {llm_name} (CONCISE MODE)")
    print(f"{'='*60}")
    print(f"\nPresentation:\n{presentation}\n")
    ...
    if not final_recommendation:
        # Ask LLM for final recommendation - WITH CONCISENESS REMINDER
        messages.append({
            "role": "user",
            "content": "Based on our conversation, please provide your final assessment and recommendations. IMPORTANT: Keep your response under 1500 characters - be concise and focus on the most critical recommendations."
        })
    ...
    # Log the response length for verification
    print(f"\n[Response length: {len(final_recommendation)} characters]")
...
def main():
    parser = argparse.ArgumentParser(description="Test an LLM on medical consultations (CONCISE mode)")
    parser.add_argument("--provider", choices=["openai", "anthropic", "google", "together", "groq", "huggingface", "openrouter", "dr7", "vertexai", "replicate"], default="openai")
    ...
    # Get LLM client
    llm_name = f"{args.provider}/{args.model}-concise"
